...
int skill_2101005(entt::entity ent)
{
    ...
    auto ski = &World::registry->emplace_or_replace<Skill>(ent, 2101005);
    if (!ski->skiw)
    {
        SDL_Log("ERROR: skill_2101005: skill not found, skipping");
        return PlayerSkill::SkillResult::None;
    }
                                    ...
                                    int full_damage)
    {
        ...
        mob->call_backs.erase(2101005);
        mob->call_backs.emplace(2101005, std::make_pair(poison_call_back, std::make_tuple(Window::dt_now + time, Window::dt_now + 500)));
    };

    Skill::cooldowns[2101005] = Window::dt_now + 500;
