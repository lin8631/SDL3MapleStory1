...
void AfterImage::load(const std::u16string &type, const std::u16string &sfx, const std::u16string &afterImage_index)
{
    ...
    if (!afterimage_node)
    {
        SDL_LogWarn(0, "AfterImage::load: Afterimage node not found");
        return;
    }
    ...
    if (!(afterimages.contains(type) && afterimages[type].contains(afterImage_index)))
    {
        ...
        if (!afterimage)
        {
            SDL_LogWarn(0, "AfterImage::load: afterimage not found, type=%s, index=%s", 
                std::string(type.begin(), type.end()).c_str(), 
                std::string(afterImage_index.begin(), afterImage_index.end()).c_str());
            return;
        }
        std::unordered_map<std::u16string, std::unordered_map<uint8_t, Wrap>> part;
        for (auto &[second, bodyPart] : afterimage->get_children())
        {
            if (Character::type_map.contains(second))
            {
                auto it = Character::type_map.find(second);
                if (it == Character::type_map.end()) continue;
                auto action = it->second;
                Wrap wrap;
                for (auto &[third, act] : bodyPart[0]->get_children())
                {
                    if (third == u"lt")
                    {
        ...
        if (!hits.contains(type + u"1"))
        {
            ...
            if (!node)
            {
                SDL_LogWarn(0, "AfterImage::load: hit.img not found");
            }
            else if (node->find_from_path(type + u"1") == nullptr)
            {
        ...
        if (!sounds.contains(sfx))
        {
            ...
            if (!node)
            {
                SDL_LogWarn(0, "AfterImage::load: weapon sound not found, sfx=%s", std::string(sfx.begin(), sfx.end()).c_str());
            }
            else
            {
