...
void Character::init()
{
    ...
    if (!character_node)
    {
        SDL_LogWarn(0, "Character::init: Wz::Character->get_root() is null!");
        return;
    }
    {
        ...
        if (!body_node)
        {
            SDL_LogWarn(0, "Character::init: body_node not found: 00002000.img");
            return;
        }
        auto head_node = character_node->find_from_path(u"00012000.img");
        if (!head_node)
        {
            SDL_LogWarn(0, "Character::init: head_node not found: 00012000.img");
            return;
        }
        for (auto stance_node : body_node->get_children())
        {
            ...
            for (uint8_t frame = 0; auto frame_node = stance_node.second[0]->get_child(std::to_string(frame)); ++frame)
            {
                ...
                {
                    uint8_t type = 0;
                    auto it = type_map.find(dynamic_cast<wz::Property<wz::wzstring> *>(frame_node->get_child(u"action"))->get());
                    if (it != type_map.end()) {
                        type = it->second;
                    }
                    uint8_t f = 0;
                ...
                {
                    auto it = type_map.find(ststr);
                    if (it == type_map.end()) {
                        SDL_LogWarn(0, "Character::init: type_map does not contain key: %s", std::string(ststr.begin(), ststr.end()).c_str());
                        continue;
                    }
                    auto type = it->second;
