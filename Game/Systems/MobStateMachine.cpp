...
optional<int> mob_hit(Attack *atk, entt::entity ent, std::optional<SDL_FPoint> hit_point)
{
    ...
    if (atk->damage > 0)
    {
        ...
        for (int i = 0; i < atk->attackCount; i++)
        {
            ...
            int baseDamage = atk->damage * r;
            int effectiveDamage = (mob->def > 0) ? (baseDamage * 100) / (100 + mob->def) : baseDamage;
            auto type = baseDamage > atk->damage ? Damage::Type::Cri : Damage::Type::Red;
            // 实际造成的伤害值（考虑防御）进行浮动
            full_damage.value() += effectiveDamage;

            mob->hp -= effectiveDamage;
            // 怪物被攻击音效
            if (atk->souw)
            {
                Sound::push(atk->souw, i * 140, -1, SoundType::PLAYER_SKILL);
            }
            else if (mob->sounds.contains(u"Damage"))
            {
                Sound::push(mob->sounds[u"Damage"], i * 140, -1, SoundType::MOB);
            }
            Damage::push(World::registry->try_get<Damage>(ent), effectiveDamage, type,
        ...
        if (mob->state != Mob::State::DIE)
        {
            if (mob->hp <= 0)
            {
                ...
                {
                    Sound::push(mob->sounds[u"Die"], 200, -1, SoundType::MOB);
                }
...
{
    std::vector<std::pair<std::u16string, unsigned int>> drops = {{u"09000000", (unsigned int)(std::rand() % 200) + 1}};
    bool has_equipment_dropped = false;
    for (auto it : mob->drops[mob->id])
    {
        auto item_type = Item::load_item_index(it);
        if (item_type == 0)
        {
            if (!has_equipment_dropped)
            {
                drops.push_back({it, 1});
                has_equipment_dropped = true;
            }
        }
        else
        {
    ...
    {
        Sound sou(u"Game.img/DropItem", 300);
        sou.type = SoundType::SFX;
        Sound::push(sou, -1);
    }
    World::zindex = true;
}
