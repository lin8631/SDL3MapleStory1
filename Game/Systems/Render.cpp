...
extern bool show_character_name;
extern bool show_pet_name;
extern int npc_show_mode;
extern int portal_show_mode;
extern int mob_show_mode;
extern float near_distance;
bool HasImGuiWindowFocus();

void render_run()
{
    auto player_tr = World::registry->try_get<Transform>(Player::ent);
    float player_x = player_tr ? player_tr->position.x : 0;
    float player_y = player_tr ? player_tr->position.y : 0;
    ...
    for (auto ent : view)
    {
        ...
        else if (auto por = World::registry->try_get<Portal>(ent))
        {
            ...
            if (portal_show_mode != 3)
            {
                float dx = tr->position.x - player_x;
                float dy = tr->position.y - player_y;
                float dist = sqrtf(dx * dx + dy * dy);
                bool is_near = dist <= near_distance;
                should_show = (portal_show_mode == 0) || (portal_show_mode == 1 && is_near);
            }
            if (should_show)
            {
        ...
        else if (auto npc = World::registry->try_get<Npc>(ent))
        {
            ...
            if (npc_show_mode != 3)
            {
                ...
                float dist = sqrtf(dx * dx + dy * dy);
                bool is_near = dist <= near_distance;
                should_show = (npc_show_mode == 0) || (npc_show_mode == 1 && is_near);
            }
            if (should_show)
            {
                if (auto nametag = World::registry->try_get<NameTag>(ent))
        ...
        else if (auto mob = World::registry->try_get<Mob>(ent))
        {
            ...
            if (mob_show_mode != 3)
            {
                bool is_attacked = mob->hit != entt::null;
                should_show = (mob_show_mode == 0) || (mob_show_mode == 2 && is_attacked);
            }
            if (should_show)
            {
                if (auto nametag = World::registry->try_get<NameTag>(ent))
...
void render_statusbar()
{
    ...
    for (auto &[key, val] : StatusBar::position_map)
    {
        ...
        if (button_ptr == &StatusBar::QuickSlot)
        {
            ...
            if (sprite_map.empty() || !sprite_map.contains(sprite_name)) continue;
        ...
        {
            if (key->second.empty() || !key->second.contains(key->first)) continue;
...
void render_cursor()
{
    if (show_imgui_demo && HasImGuiWindowFocus()) return;
