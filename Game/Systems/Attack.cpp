#include "Attack.h"
// 在 Attack.cpp 中引入 monster/Character 组件头以判断来源类型
#include "Components/Mob.h"
#include "Components/Character.h"
// 用于怪物对玩家伤害的浮动
#include <random>
// RNG for dynamic damage fluctuation when monsters attack players
#include <random>
#include "Collision.h"
#include "Core/Core.h"
...
void attack_reactor(Attack *atk)
{
    ...
    for (auto ent : World::registry->view<Reactor>())
    {
        ...
        for (auto &event : reactor->a[reactor->index].event)
        {
            ...
            {
                Sound::push(reactor->sounds[reactor->index], 0, -1, SoundType::SFX);
                reactor->hit = true;
                return;
            }
        }
...
int attack_player(Attack *atk, entt::entity src, std::optional<SDL_FPoint> p)
{
    ...
    int full_damage = 0;
    // Monster-to-Player path: monsters deal base damage with ±20% fluctuation
    auto mob_comp = World::registry->try_get<Mob>(src);
    if (mob_comp != nullptr)
    {
        atk->damage = 100;
        atk->min_damage = 0.8f;
